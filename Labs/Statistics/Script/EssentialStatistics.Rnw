...
Statistics, or more broadly data science, deals with the visualization, summary and interpretation of data. I will provide here an introduction to what I think are the four pillars of statistical methods for a quantitative researcher:
...
\marginnote{The difference to inferential statistics is not always entirely clear cut, but the general idea is that the focus of inferential statistics is to find out something about a model or assumption (is the hypothesis true ...). Predictive methods often don't even make such assumptions. Their focus is on finding ways to transform data into predictions, without the intermediate step of finding out the "data-generating process".}
...
\paragraph{Experimental design} covers all aspects of generating data, but in particular questions such as "which variables need to be measured?", "how many data do we need?", "how should we optimally vary the variables of interest in an experiment"?
...
In this primer, all examples will be calculated with R; however, I will give no further introduction to R. If you need one, follow  \href{http://biometry.github.io/APES/R/R10-gettingStarted.html}{this link} for an introduction, in including help on how to install the software.

\chapter{Data, descriptive statstics and visualization}

Descriptive statistics deals with the summary and visualization of data. Before we speak about those methods, it will be useful to shortly discuss what data is and what properties it may have.\footnote{For the moment, assume that we have some data. We will speak about how to create data in the later chapter~\ref{cha: design of experiments} on experimental design.} 

\section{Data}

A typical dataset consists of multiple observations of number of variables (e.g. temperature, precipitation, growth).\footnote{Of course, there are other data structures, but this is the most common one.} You can think of this situation as a spreadsheet where the columns are the variables and the rows are the observations.
...
Summary statistics are numerical calculations that summarize a dataset. \marginnote{A simple summary statistics you should be familiar with is the mean}.

\subsection{Summarizing distributions - moments and quantiles}

Imagine we have a single, continous variable with a large number of observations - hence, we have a distribution of values (see below).
...
samples = mix * rnorm(2000) + (1-mix)*rnorm(2000, mean = 2.5, sd=0.5)

distr = (dnorm(parameter) + dnorm(parameter, mean = 2.5, sd=0.5)) /2

plot(parameter,distr, type = "l", ylab = "Density", xlab = "Observed value", col = "red", lwd = 2, ylim = c(0,0.5))
hist(samples, breaks = 100, add = T,freq = F, col = "#99999930")
...
How can we summarize the properties of this distribution. Some basic properties would be the minimum and the maximum, as well as the model (the maximum of the distribution, i.e. value with the highest density of observations). There are two further central summaries that are always calculated - moments and quantiles.

The n-th moment is defined as the average across the distribution, taking each observed value to the poewr of n. 
...
For calculating descriptive statistics in R, see \href{http://www.uni-kiel.de/psychologie/rexrepos/rerDescriptive.html}{here}
...
There are number of excellent introductions to graphics with R, so that I won't bother to provide details here. I recommend to look at
