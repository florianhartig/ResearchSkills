...
\marginnote{We will speak about how to create data in the later chapter~\ref{cha: design of experiments} on experimental design.} 
...
\marginnote{Of course, there are other data structures, but this is the most common one.} You can think of this situation as a spreadsheet where the columns are the variables and the rows are the observations.

A typical dataset consists of multiple observations of number of variables (e.g. temperature, precipitation, growth).

\marginnote{The response variable is the variable for which we try to understand how it responds to other factors.} 
...
\marginnote{A simple summary statistics you should be familiar with is the mean}
...
\subsection{Correlatins - summarizing the dependence between continous variables}
